...
;;
;; Q: why this interchange makes the program run slowly?
         ...
         (flatmap
          (lambda (new-row)
            (map (lambda (rest-of-queens)
                 ...
                 (queen-cols (- k 1))))  ;; A: because we will evaluate (queens-cols (- k 1)) again and again
            (enumerate-interval 1 board-size)))))
  (queen-cols board-size))

;; tests
(queens 8)    ;; it will take a long long time :(
